...
class AcceptorKorni3(object) :
    ...
    def _write(self, register: str, promise, accepted): # return void
        ...
        # , '--ignore'
        p2 = subprocess.run(['korni3', 'insert', self.korni3container, self.tableName], input=json.dumps(data).encode('utf-8'),
        ...
        try:
            ...
            for record in self.cur.fetchall():
                ...
                def _sendPrepareResponse():
                    ...
                    p1 = subprocess.run(['korni3', 'insert', self.korni3container, self.presptable], input=json.dumps(data).encode('utf-8'),
                                        stdout=PIPE, stderr=PIPE)
                    if p1.returncode > 0:
                        print('ERROR Acceptor checkPrepareRequestsHandler ', p1.stdout.decode('utf-8'))
        ...
        try:
            ...
            for record in self.cur.fetchall():
                ...
                def _sendAcceptResponse():
                    ...
                    p = subprocess.run(['korni3', 'insert', self.korni3container, self.aresptable],
...
class ProposerKorni3(object):
    ...
    def _write(self, register: str, ballot, functionId: str, nextValue, state, reqid):
        data = {"id": json.dumps([register,ballot]),
                ...
                "reqid": reqid}
        # , '--ignore'
        p2 = subprocess.run(['korni3', 'insert', self.korni3container, self.tableName],
                            input=json.dumps(data).encode('utf-8'),
                            stdout=PIPE, stderr=PIPE)
    ...
    def _sendPrepareRequest(self, acceptorPub: str, register: str, ballot):
        data = {"id": register, "type": 'prepare', "aid": acceptorPub, "b": ballot,
                "handled": False }
        p1 = subprocess.run(['korni3', 'insert', self.korni3container, self.preqtable],
                            input=json.dumps(data).encode('utf-8'),
                            stdout=PIPE, stderr=PIPE)
    ...
    def accept2(self, reg, b, breq, F, fId, state, reqid, isConfirm: bool):
        if str(b) == str(breq):
            ...
            subprocess.run(['korni3', 'insert', self.korni3container, self.outtable, '--ignore'],
                           input=json.dumps(data).encode('utf-8'),
                           stdout=PIPE, stderr=PIPE)
